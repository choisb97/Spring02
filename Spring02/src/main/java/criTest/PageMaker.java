...
//------------------------------------------------------------------------------------------------------
	...
	// ** 배열Type인 check를 Map으로
	// => UriComponents에서 Multi Value 처리 :  queryParams(MultiValueMap<String, String> params) 
		
	// ** MultiValueMap
	// => 키의 중복이 허용됨 즉, 하나의 키에 여러 값을 받을 수 있음
	// => new LinkedMultiValueMap()으로 생성, add("key","value")

	// ** Map (키 중복 허용 안 됨) 과 비교
	// => HashMap : 순서보장 안 됨
	...
	public String searchQuery(int currPage) {
		...
		// => check에 선택한 값이 있는 경우에만
		// 배열 check의 원소들을 Map으로
		...
		UriComponents uriComponents = UriComponentsBuilder.newInstance()
				...
				.queryParam("keyword", cri.getKeyword()).queryParams(checkMap)
				.build();
