...
<head>
	<meta charset="UTF-8">
	<title>** kakaoMap02_여러 주소를 좌표로 변환하여 위치와 설명 표시 **</title>
...
<h2>** Map Test02 : 여러 주소를 좌표로 변환하여 클릭 시 위치와 설명 표시 **</h2>
<br><hr>
...
var addrs = [
		...
		{ content : '안양',   
		  addr : '경기 안양시 만안구 안양 4동 676 - 91 (안양로 303) 안양메쎄타워 2층' }
	];
...
//    배열 addrs가 addr 하나의 데이터 속성만 있다면 무관하지만 
//    위의 경우처럼 content가 있는 경우에는 이 content 값은 마지막 값만 볼 수 있게 된다
...
// => 해결은 여러가지가 있겠지만, 데이터의 구조와 특징에 따라 구현하는 것이 좋을 듯
...
// => 오류 : JS의 콜백함수는 비동기 실행을 해서 일정 순서대로 실행되지 않기 때문에
//           content 값의 순서와 다르다.
// => 결론 : DB에 자료를 보관할 때 주소와 좌표를 모두 보관해야 정확하게 info를 DB에서 읽어 표시할 수 있다. 
