...
<head>
	...
	<script>
		...
		function connect() {
			...
			wsocket.onopen = onOpen; // 연결 시 콜백 함수
			wsocket.onmessage = onMessage; // 서버로부터 메시지 도착 시 콜백 함수
			wsocket.onclose = onClose; // 종료 시 콜백 함수
		} //connect
		...
		function send() {
			...
			$('#message').val(''); // 전송 후 Clear
		}
		function appendMessage(msg) {
			// Message Area에 Message를 추가  & scroll 이동
		...
		$(function() {
			...
			$('#message').keypress(
					function(event) {
						...
						//    그러나 브라우저 마다 차이가 있을 수 있기 때문에 아래 코드 사용 
						console.log('event.keyCode => ' + event.keyCode + 'event.which => ' + event.which);
						var keycode = (event.keyCode ? event.keyCode : event.which);
						// event.keyCode가 있으면 event.keyCode 사용, 아니면 event.which 사용
						// enter 키 입력 시 전송되도록  
						if (keycode == '13') {
							send();
						}
