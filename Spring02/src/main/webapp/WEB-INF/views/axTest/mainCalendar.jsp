...
<head>
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar ({
			...
			events : test,
			// => 배열 test의 Data(즉, 일정)를 화면에 그려주는 function()
			...
			//드래그 & 드롭 : 일정 수정
	 		eventDrop : function(event, delta, revertFunc){
				...
				$.ajax ({
					...
					//cache : false;	//ajax로 통신 중 cache가 남아서 갱신된 데이터를 받아오지 못 할 경우 사용
					//async : false;	//false로 선언 시 ajax 결괏값이 끝난 다음에 함수가 진행됨
	...
	/* 	console.log("** csrf => "+csrf);
		...
		$.ajax ({
			type : 'POST',
			url : "calendarInsert", // 저장하기 버튼 눌렀을 시 이동하는 컨트롤러 주소
			data : {
				caid : caid,
			...
			async : false,	// false로 선언 시 비동기식 처리 -> ajax결과값이 끝난 다음에 함수가 진행됨	
	/* 		beforeSend : function(xhr) {
				xhr.setRequestHeader(csrfHeader, csrf);
				}, */	
			success: function(data) {
	...
	#calendar {
		max-width: 700px; /* calendar의 width 최댓값 : 이 범위내에서 100% 자동조절 */
		margin: 0 auto;
	}
	</style>
</head>
